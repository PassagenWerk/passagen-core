...
Core 内置 evidence、summary、full、reduce、repair 和 outline 的版本化模板，并支持调用方
提供外部路径。模板使用 Python `string.Template`：

```text
evidence: $schema, $chunk
summary:  $schema, $identity, $evidence
full:     $schema, $identity, $paper
reduce:   $schema, $evidence
repair:   $schema, $validation_error, $candidate
outline:  $schema, $summary
```

字面 `# Passagen Core Operations
...
Authorization header，不得写入配置输出、普通日志、数据库或诊断 artifact。
 ...
 写为 `$`。Core 校验模板可读性、占位符和输出 Schema。Evidence cache key 包含模板
